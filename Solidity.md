...
- calldata: схож с memory, но доступен только для external функций.
...
----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
#### ГЛОБАЛЬНЫЕ ПЕРЕМЕННЫЕ И ВСТРОЕННЫЕ ФУНКЦИИ:  
...
- transfer - ф-ция перевода средств, при возникновении ошибки выбрасывается исключение, после чего код не будет выполняться.  
- send - ф-ция перевода средств, ошибка передачи не вызывает исключения и возвращает true/false. Код продолжит выполняться.
- call.value().gas()() - ф-ция перевода стредств, ошибка передачи не выбрасывает исключение и возвращает true/false. Код будет выполнен, но функции вызова для передачи подвержены атакам реентерабельности. 
...
Для того, чтобы наш контракт мог общаться с другим контрактом (владельцем которошго являемся не мы), сперва необходимо определить interface.  
