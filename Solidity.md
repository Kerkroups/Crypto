...
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
#### ВИДЫ ХРАНИЛИЩ В SOLIDITY, ХРАНЕНИЕ ДАННЫХ В БЛОКЧЕЙН, ДОСТУП К STORAGE:   
...
EVM хранит переменне в массиве ячеек памяти называемые "slots". Ячейка слота равна 32 bytes. Данные хранться в очереди их объявления в коде.  

uint переменные преобразуются в формат HEX. Если для хранения переменной не нужно 32 байт, наприме ждля типа address, а следующим идет переменная типа bool, эти переменные будут храниться в одном слоте памяти. Значение переменной типа bool будет записанно перед значением переменно типа address. Если в одной ячейке хранятся две переменные, то EVM будет обрабатывать эти значения с права на лево.  

Данные массивов фиксированной длинны в EVM хранятся последовательно и соответственно типу данных.  

Хранение динамических массивов в EVM:
...
**constant** значение этих переменных устанавливается перед компиляцией кода и не может быть моджифицированно после. Расходуют меньше gas. Должны определяться в верхнем регистре. Не хранятся в STORAGE.  ```uint256 public constant UINT_CONSTANT = 123;```  
